...
function LocationAutocomplete({ value, onChange, onSelect, placeholder }) {
  ...
  useEffect(() => {
    ...
    // Debounce so we don't hit Nominatim on every keystroke (their usage
    // policy asks for roughly max 1 request/second).
        ...
        <div className="absolute top-[46px] left-0 w-full bg-white rounded-xl border border-[#ececec] shadow-lg z-30 max-h-[260px] overflow-y-auto py-1">
          ...
          {loading ? (
            ...
            suggestions.map((place) => (
              ...
              >
                <FaMapMarkerAlt className="text-[#2563eb] text-[12px] mt-0.5 shrink-0" />
